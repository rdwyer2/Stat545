...
gapminder %>% group_by(country) %>% 
  summarize(change = pop[year==1972] - pop[year==1962])
```
...
gapminder %>% 
  ...
  ggplot(aes(x = year, y = pop/1000000)) +
    geom_line(group = "country", color = log(max_gdp)) +
    facet_wrap(~continent) +
    scale_y_log10("Population (millins)", breaks = c(.1, 1, 10, 100, 1000), 
    ...
    theme(axis.test.x = element_text(angle = 90),
        plot.title = element_test(hjust = .5),
